...
- **MissingDataPrompt i18n + catalog coverage** (#363): `MissingDataPrompt` now resolves its default title/description through `useGdsTranslation()` (`gds.state.missingData.title` / `gds.state.missingData.description`, added to all 9 locale packs) instead of hard-coded English, and gained `'use client'`. The `state-blocks` catalog demo renders the prompt with a required-fields list, a call-to-action slot, and a `role="status"` recovery variant. Consumer-passed `title`/`description` are unchanged.
- **Form control family catalog coverage + i18n** (#364): the `forms` and `form-field` catalog demos now mount `GdsSegmentedControl` (default, disabled, and many-item overflow), `GdsSlider` (1–10 plus a min-equals-max boundary), `GdsRatingScale` (1–5), and `GdsWizardStepper` (first/mid/last steps). Wizard/slider/rating default copy (`gds.form.wizard.back|next|finish|progress|optional`, `gds.form.slider.label`, `gds.form.rating.aria`) now routes through i18n in all 9 packs while keeping the English string as the prop default, so no public API changes.
- **Overlay Dialog/SidePanel alias coverage + i18n** (#365): the `modals` and `drawers` catalog demos now open `GdsDialog` and `GdsSidePanel` (open/close, focus trap, focus return, opaque surface). The overlay close-button `aria-label` now routes through the new `gds.overlay.close` key (added to all 9 packs) instead of Mantine's built-in English default.
- **Chart-wrapper family catalog coverage + i18n** (#366): the `reporting-contracts` catalog demo now renders all seven wrappers (`GdsAreaChart`, `GdsSparkline`, `GdsLongitudinalChart`, `GdsBenchmarkBarChart`, `GdsRadarChart`, `GdsMaturityRadarChart`, `GdsGaugeChart`) in populated, empty, and loading states so the accessible table fallback is exercised per theme. The chart table-fallback headers now resolve through i18n (`gds.chart.table.label|value|secondaryValue|group`, all 9 packs), the existing reporting legend swatches use `getGdsSeriesColor` instead of raw Mantine tokens, and a focused test asserts the benchmark wrapper mirrors its input into the table fallback.
...
- Added [COMPLIANCE_TOOLKIT.md](COMPLIANCE_TOOLKIT.md) as the canonical CI and local enforcement contract for `@doneisbetter/gds-eslint-config` and `@doneisbetter/gds-compliance`.
