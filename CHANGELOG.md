...
- **Automated Projects v2 board sync on release** (#431): new `.github/workflows/board-sync.yml` runs `board:sync-release` in CI (where `gh` is preinstalled) after **GDS Release Bundles** completes, moving every closed issue's board card (project #11) to **Done** — previously a manual `gh`-only step that drifted after each release. Uses `workflow_run` (not `push: tags`, which a `GITHUB_TOKEN`-pushed tag never fires) and an opt-in `GDS_PROJECT_TOKEN` PAT secret for org-level Projects v2 writes; if the secret is absent the job warns and skips rather than failing the release. Documented in `RELEASE_PUBLISH.md`.
