...
- published baseline (GitHub Packages): `6.7.0`
- current repository line: `6.7.0`
...
For the current `6.7.0` line, the install contract is (requires the `.npmrc` block above):

```bash
npm install @sovereignsquad/gds@6.7.0
npm install -D @sovereignsquad/gds-eslint-config@6.7.0 @sovereignsquad/gds-compliance@6.7.0
```
...
Do not mix pre-3.0 package lines with `6.7.0` packages in the same consumer dependency graph.
