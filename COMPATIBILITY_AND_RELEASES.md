...
- published baseline (GitHub Packages): `3.12.0`
- current repository line: `3.12.0`
...
For the current `3.12.0` line, the install contract is (requires the `.npmrc` block above):

```bash
npm install @sovereignsquad/gds@3.12.0
npm install -D @sovereignsquad/gds-eslint-config@3.12.0 @sovereignsquad/gds-compliance@3.12.0
```

Granular consumers should use the same version across every package:

```bash
npm install @sovereignsquad/gds-theme@3.12.0 @sovereignsquad/gds-core@3.12.0 @sovereignsquad/gds-admin@3.12.0
npm install -D @sovereignsquad/gds-eslint-config@3.12.0 @sovereignsquad/gds-compliance@3.12.0
```

Do not mix pre-3.0 package lines with `3.12.0` packages in the same consumer dependency graph.
