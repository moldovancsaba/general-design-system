...
- published baseline (GitHub Packages): `4.1.0`
- current repository line: `4.1.0`
...
For the current `4.1.0` line, the install contract is (requires the `.npmrc` block above):

```bash
npm install @sovereignsquad/gds@4.1.0
npm install -D @sovereignsquad/gds-eslint-config@4.1.0 @sovereignsquad/gds-compliance@4.1.0
```
...
Do not mix pre-3.0 package lines with `4.1.0` packages in the same consumer dependency graph.
