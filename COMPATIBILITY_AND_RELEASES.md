...
- published baseline (GitHub Packages): `3.14.2`
- current repository line: `3.14.2`
...
For the current `3.14.2` line, the install contract is (requires the `.npmrc` block above):

```bash
npm install @sovereignsquad/gds@3.14.2
npm install -D @sovereignsquad/gds-eslint-config@3.14.2 @sovereignsquad/gds-compliance@3.14.2
```
...
Do not mix pre-3.0 package lines with `3.14.2` packages in the same consumer dependency graph.
