...
- published baseline (GitHub Packages): `5.0.3`
...
For the current `5.0.3` line, the install contract is (requires the `.npmrc` block above):

```bash
npm install @sovereignsquad/gds@5.0.3
npm install -D @sovereignsquad/gds-eslint-config@5.0.3 @sovereignsquad/gds-compliance@5.0.3
```
...
Do not mix pre-3.0 package lines with `5.0.3` packages in the same consumer dependency graph.
