...
- published baseline (GitHub Packages): `3.14.17`
...
For the current `3.14.17` line, the install contract is (requires the `.npmrc` block above):
...
Do not mix pre-3.0 package lines with `3.14.17` packages in the same consumer dependency graph.
