...
- published baseline (GitHub Packages): `4.1.11`
...
For the current `4.1.11` line, the install contract is (requires the `.npmrc` block above):

```bash
npm install @sovereignsquad/gds@4.1.11
npm install -D @sovereignsquad/gds-eslint-config@4.1.11 @sovereignsquad/gds-compliance@4.1.11
```
...
Do not mix pre-3.0 package lines with `4.1.11` packages in the same consumer dependency graph.
