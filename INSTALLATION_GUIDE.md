...
- current stable package line: `3.4.0`
...
Preferred `3.4.0` runtime package after the release gate opens:
...
For the `3.4.0` release cutover, fallback assets must use tag `gds-v3.4.0` and must not be announced as the canonical path once npm verification passes.
