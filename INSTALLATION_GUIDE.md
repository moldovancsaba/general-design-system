...
Preferred `3.5.0` runtime package after the release gate opens:
...
```bash
npm install -D @doneisbetter/gds-eslint-config@3.5.0 @doneisbetter/gds-compliance@3.5.0 @doneisbetter/gds-a11y@3.5.0
```
...
For the `3.5.0` release cutover, fallback assets must use tag `gds-v3.5.0` and must not be announced as the canonical path once npm verification passes.
