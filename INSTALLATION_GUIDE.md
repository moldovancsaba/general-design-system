...
- current stable package line: `3.0.1`
...
Preferred `3.0.1` runtime package after the release gate opens:
...
For the `3.0.1` release cutover, fallback assets must use tag `gds-v3.0.1` and must not be announced as the canonical path once npm verification passes.
