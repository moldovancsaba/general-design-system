...
- current stable package line: `3.4.5`
...
Preferred `3.4.5` runtime package after the release gate opens:
...
For the `3.4.5` release cutover, fallback assets must use tag `gds-v3.4.5` and must not be announced as the canonical path once npm verification passes.
