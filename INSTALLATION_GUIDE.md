...
- current stable package line: `6.1.0`
...
Preferred `6.1.0` runtime package:
...
```bash
npm install -D @sovereignsquad/gds-eslint-config@6.1.0 @sovereignsquad/gds-compliance@6.1.0 @sovereignsquad/gds-a11y@6.1.0
```
...
2. Switch to the recommended umbrella at the current version: `npm install @sovereignsquad/gds@6.1.0` (it re-exports `gds-core`, `gds-theme`, and `gds-admin`, so you depend on one package instead of several). If you prefer to keep the split packages, install `@sovereignsquad/gds-core@6.1.0` / `@sovereignsquad/gds-theme@6.1.0` instead — both resolve from GitHub Packages.
