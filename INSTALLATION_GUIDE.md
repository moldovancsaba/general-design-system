...
- current stable package line: `5.0.0`
...
Preferred `5.0.0` runtime package:
...
2. Switch to the recommended umbrella at the current version: `npm install @sovereignsquad/gds@5.0.0` (it re-exports `gds-core`, `gds-theme`, and `gds-admin`, so you depend on one package instead of several). If you prefer to keep the split packages, install `@sovereignsquad/gds-core@5.0.0` / `@sovereignsquad/gds-theme@5.0.0` instead — both resolve from GitHub Packages.
