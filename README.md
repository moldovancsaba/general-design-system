...
- **Design Rule Profiles**: [docs/DESIGN_RULE_PROFILES.md](docs/DESIGN_RULE_PROFILES.md) — Computed 60-30-10 color-proportion classification, named type-scale ratios, hue-angle color-harmony classification, lint/CLI/runtime enforcement, rendered-pixel measurement, and the Theme Lab panel that displays it all live.
...
- **Theme Governance**: [THEME_GOVERNANCE.md](THEME_GOVERNANCE.md) — Brand extension, dark-mode defaults, white-label, flat-surface, and tenant-theme rules, plus the `data-gds-fixed-tone` element-level opt-out from the theme-preset repaint.
...
- `npm run audit:design-rule-coverage` — rendered color-proportion sample (issue #649): for every preset × scheme across a fixed four-route sample, captures each visible element's rendered background-color area, classifies it against issue #644's dominant/secondary/accent role split, and writes area-weighted percentages to `audit/design-rule-coverage.json`; a real measurement artifact, not a CI gate (see issue #650)
