...
- **Locale Bridge**: `getGdsMessages(locale)` and `GdsLocale` from `@doneisbetter/gds-core`, plus `gdsLocaleMetadata`, `getGdsLocaleMetadata(...)`, `isGdsRtlLocale(...)`, and `getGdsLocaleIdsByScript(...)` from `@doneisbetter/gds-theme` — canonical bridges for host i18n systems, locale-aware direction, script coverage, and GDS-owned semantic labels. Consumers must centralize language resources instead of embedding per-language prose in UI components.
