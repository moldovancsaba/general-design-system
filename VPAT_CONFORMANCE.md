...
**Name of Product/Version:** General Design System (GDS) — `@sovereignsquad/gds` and the granular `@sovereignsquad/gds-*` packages, release line `3.14.2`
...
| **1.4.3** Contrast (Minimum) | Supports | Mandatory registry mapping **and** the release-blocking contrast gate across all shipped themes. Provider: governed token pairs. Consumer: contrast of their own brand tokens (self-checkable via the `gds-a11y` contrast gate). Evidence: `verify:theme-accessibility`. |
...
- **Not yet gate-asserted (scored Partially Supports above):** Text Spacing (1.4.12), Focus Not Obscured (2.4.11), and Target Size (2.5.8) rely on design conventions rather than a dedicated release gate. Real per-component WCAG fg/bg numeric scoring against the component-painting palette is tracked in issue #456.
