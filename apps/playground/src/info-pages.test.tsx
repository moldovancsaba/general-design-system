...
describe('playground overview page', () => {
  it('frames the site as the official reference and live proof', () => {
    ...
    expect(screen.getByText('Official reference and live proof')).toBeTruthy();
    ...
    expect(screen.getAllByRole('link', { name: 'Open live proofs' }).length).toBeGreaterThan(0);
  ...
  it('renders fully localized install copy for de and fr locales', () => {
    ...
    expect(screen.getByText('Öffentlicher 6.3.0-Installationspfad')).toBeTruthy();
    expect(screen.getAllByText(/@sovereignsquad\/gds@6.3.0/).length).toBeGreaterThan(0);
    ...
    expect(screen.getByText('Parcours d’installation public 6.3.0')).toBeTruthy();
  });
