...
// Statically imported (renders on first paint of / and /themes) from its own
// subpath, not the main barrel, so consumers who don't use it don't bundle it.
 ...
 *
 * Uses `%count%`, not `{count}` — the phrase extractor rejects strings containing braces.
 */
function withComponentCount(phrase: string): string {
  // Regex, not the literal string, so the extractor doesn't collect '%count%' as its own phrase.
...
Full rules: https://sovereignsquad.github.io/general-design-system/ai`;
  ...
  // /ai has no runtime gate, so auto-start is safe.
...
export function OverviewPage({
  ...
  // Home is a theme-trust route, only ever visited as "/?locale=xx", so the
  // no-query auto-start guard keeps the tour from firing during that check.
  ...
  const homeSteps: GdsTourStep[] = [
    ...
    },
    {
      ...
      // Must stay a string literal — the phrase extractor doesn't see template literals.
      // %count% is substituted at render from GDS_PUBLIC_COMPONENT_COUNT.
...
export function TokensPage({
  ...
  return (
    ...
    >
      ...
      {/*
        Two atmosphere scales: --gds-vibe-hero (full-width band, 135deg ramp) and
        --gds-vibe-swatch (small surfaces). Using hero's ramp on a swatch renders a hard
        diagonal instead of a wash. Values below read live from the theme.
      */}
      <ReferenceSection
