...
import { GDS_PUBLIC_COMPONENT_COUNT } from './generated-component-census';

/**
 * Substitutes the generated component count into a translatable sentence.
 *
 * `%count%` rather than `{count}`: the phrase extractor rejects strings containing braces, so
 * a brace placeholder would drop the sentence from every locale. The translated copy keeps the
 * placeholder, and the number lands after translation.
 */
function withComponentCount(phrase: string): string {
  // A regex, not the string '%count%': the extractor collects every string literal, so a bare
  // one would enter the phrase corpus as its own "phrase" and be translated into nonsense
  // (German returned "%zählen%").
  return phrase.replace(/%count%/, String(GDS_PUBLIC_COMPONENT_COUNT));
}
...
export function OverviewPage({
  ...
  const homeSteps: GdsTourStep[] = [
    ...
    },
    {
      ...
      // Issue 605. This opened with a hardcoded '250+'. True at the time (289) and written by
      // hand, so it drifted in both directions: understating the system, and one deletion away
      // from becoming a false promise.
      //
      // It could not be a template literal — the phrase extractor only sees string LITERALS,
      // so a template would silently drop this sentence from all eight locales. The answer is
      // a placeholder the extractor accepts: the sentence stays one literal (extracted and
      // translated with %count% intact), and the number is substituted at render from
      // GDS_PUBLIC_COMPONENT_COUNT — generated by the same census verify:component-catalog-
      // parity enforces against. Deleting the number was the wrong fix; this is the right one.
      body: withComponentCount('%count% governed, accessible React components, design tokens, and runtime systems — composed in every product, never reinvented.'),
      placement: 'top',
    },
    {
      id: 'home-start',
