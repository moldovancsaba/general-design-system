...
GDS 4.0.0 delivered the first-class `Class USA` theme and the missing ClassScout primitives required to ship on pure GDS with no app-level forks, raw design values, or app-local chart/control shims. The current stable line is **6.3.0**; see B11–B18 below for what shipped since 4.0.0.
