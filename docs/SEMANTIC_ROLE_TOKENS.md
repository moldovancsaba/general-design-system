...
This layer defines the **structural contract roles** (backgrounds, text, border, on-inverse) — the ones the issue names (`bg`/`surface`/`on-*`/`border`) and the ones the readability contract depends on. It deliberately does **not** define the **decorative / state / accent** roles (`--gds-brand-accent`, `--gds-state-*`, `--gds-focus-ring`, `--gds-badge-*`, `--gds-price`, `--gds-support`, …) at the default layer. Those stay **brand/preset-driven** and fall back per component as before, because their hue is a brand decision (an accent is intentionally violet under one theme, terracotta under another) rather than a fixed default. `createBrandTheme(...)` and the `class-usa` / `gold-athlete` presets continue to define the full role set including those.
